...
from TheDigger_src.utils.exceptions import TheDiggerException, HostHandlerException
from TheDigger_src.utils.request_handler import RequestHandler
from TheDigger_src.utils.logger import SystemOutLogger
from TheDigger_src.utils.help_utils import HelpUtilities
from TheDigger_src.lib.fuzzer import URLFuzzer
from TheDigger_src.lib.host import Host
from TheDigger_src.lib.scanner import Scanner, NmapScan, NmapVulnersScan, VulnersScanner
from TheDigger_src.lib.sub_domain import SubDomainEnumerator
from TheDigger_src.lib.dns_handler import DNSHandler
from TheDigger_src.lib.waf import WAF
from TheDigger_src.lib.tls import TLSHandler
from TheDigger_src.lib.web_app import WebApplicationScanner
...
https://github.com/Jistrokz/TheDigger
-------------------------------------------------------------------
...
@click.option("--vulners-path", default=os.path.join(MY_PATH, "utils/misc/vulners.nse"),
              help="Path to the custom nmap_vulners.nse script."
                   "If not used, TheDigger uses the built-in script it ships with.")
...
#This is included for the future. This feature can be turned on to send request at intervals
...
@click.option("-o", "--outdir", default="TheDigger_scan_results",
...
def main(target,
    ...
    try:
        ...
        except TheDiggerException as e:
        ...
        if tor_routing:
            ...
            except TheDiggerException as err:
                print("{}{}{}".format(COLOR.RED, str(err), COLOR.RESET))
                exit(3)

        main_loop = asyncio.get_event_loop()

        logger.info("{}### TheDigger has started Digging ###{}\n".format(COLOR.GRAY, COLOR.RESET))
